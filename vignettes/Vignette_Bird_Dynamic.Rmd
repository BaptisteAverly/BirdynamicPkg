...
FormattedMortality$parc <- bdy_clean_names(FormattedMortality$parc) # TO INTEGRATE DIRECTLY IN CHECK MORTALITY FUNCTION!!!!!!!!!!!!!
...
ggplot()+
  ...
  geom_sf(data=parcs_L93, fill="black")+
  theme_minimal()
...
We can now apply the [bdy_apportionning()] function to run the apportionning calculations to set how mortality data should be distributed among colonies. The output includes absolute and relative weights at both colony and group of colonies scales. The value used to split mortality data in `bdy_process_mortality` is the relative weight of mortality between the parcs for each group of colonies (`RW_group`).
```{r apportionning}
...
leafmap <- bdy_plot_map(mod_outputs, result_table, parcs_L93)
leafmap
...
bdy_plot_windfarm_impact(result_table, FormattedMortality)
```
