...
bdy_pretty_result_table <- function(Raw_ResTables, type){
  ...
  ResTable <- ResTable %>%
    mutate(
      ...
      Impact_Relatif = paste0(round(RelImpact_95, 2), "% [", round(RelImpact_2.5, 2), "-", round(RelImpact_med,2), "-", round(RelImpact_97.5, 2), "]"),
  ...
  # Add max impact
  if(type=="national"){
    ResTable_MaxIR <- Raw_ResTables$Tableau_Subpop %>% group_by(Species) %>%  slice(which.max(RelImpact_95))
    ResTable_MaxIR$Text <- ifelse(ResTable_MaxIR$RelImpact_95==0, 0, paste0(round(ResTable_MaxIR$RelImpact_95, 2), "% (Groupe de colonies : ", ResTable_MaxIR$GrColo, ")"))
    ResTable_MaxExt <- Raw_ResTables$Tableau_Subpop %>% group_by(Species) %>%  slice(which.max(Ext_Relative))
    ResTable_MaxExt$Text <- ifelse(ResTable_MaxExt$Ext_Relative==0, 0, paste0(round(ResTable_MaxExt$Ext_Relative, 2), "% (Groupe de colonies : ", ResTable_MaxExt$GrColo, ")"))
