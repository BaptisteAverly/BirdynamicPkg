...
#'                }
#' @param windfarms sf table where each row is a wind farm of interest.
#'                Must contain the columns:
#'                \itemize{
...
bdy_get_distances <- function(colonies,windfarms,costMatrix,doShpa=T,progress=NULL){
  ...
  colnames(eucl_dist) <- windfarms$NAME

  n_colo <- nrow(colonies)
  n_windfarms <- length(windfarms$NAME)

  ## Calculate shortest path between colonies and wind farms
  goals <- st_coordinates(st_centroid(colonies)) %>% suppressWarnings()
  iMax <- n_windfarms

  # Object to store shortest path distances between each colony and each windfarm
  shpa_dist <- eucl_dist
  shpa_dist[] <- NA
  rownames(shpa_dist) <- colonies$colony_code
  colnames(shpa_dist) <- windfarms$NAME

  if(!is.null(progress)){
    progress$set(0.05, detail = paste0("Windfarm 1  / ",iMax))
  }

  if(doShpa){
    for(i in 1:iMax){

      #coordinates of the current windfarm
      origin <- st_coordinates(st_centroid(windfarms))[i,] %>% t()

      #which façade is the park in
      seafront <- windfarms$seafront[i]
      ...
      #colonies which are not on the same façade as the windfarm get very high shortest path distances (no influence)
