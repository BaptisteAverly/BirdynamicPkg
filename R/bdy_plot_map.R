...
#' @param windfarms sf object of windfarms
...
bdy_plot_map <- function(mod_out, Raw_ResTables, windfarms, BufferForaging=NULL){
  ...
  for(SP in 1:length(mod_out)){
    ...
    # Get if colony in range of a windfarm
    Dist <- mod_out[[SP]]$distanceBIN %>% data.frame()
    Dist$Windfarm_foraging <- apply(Dist, 1, function(x) paste0(names(data.frame(mod_out[[SP]]$distance))[x == T], collapse = ";"))

    # Distance to windfarms
    Dist_qtt <- mod_out[[SP]]$distance %>%
      data.frame() %>%
      mutate(
        Dist_min = apply(., 1, min, na.rm=T),
        Windfarm_min = names(.)[apply(., 1, which.min)]
      )
    ...
    Colonies_SP$Windfarm_foraging <- Dist$Windfarm_foraging[match(Colonies_SP$colony_code, rownames(Dist))]
    Colonies_SP$Dist_min <- Dist_qtt$Dist_min[match(Colonies_SP$colony_code, rownames(Dist_qtt))]
    Colonies_SP$Windfarm_min <- Dist_qtt$Windfarm_min[match(Colonies_SP$colony_code, rownames(Dist_qtt))]
    ...
    GrColo_SP$Popup <- paste0(
      ...
      "<p style='color:FireBrick'><b>Impact relatif sur les tendances : </b>", round(Res_match$RelImpact_95,2), "% [", round(Res_match$RelImpact_2.5,2), " - ", round(Res_match$RelImpact_med,2),  " - ", round(Res_match$RelImpact_97.5,2), "]<br>",
    ...
    Leaf <- Leaf %>%
      ...
      addCircleMarkers(data=GrColo_SP, radius=GrColo_SP$LeafSize, color=ifelse(GrColo_SP$Windfarm_foraging=="aucun", "#7fbc41", "#8e0152"), fill=T, fillColor = ColPal(GrColo_SP$RelImpact), label=paste0("Cliquez pour les résultats du groupe de colonies n°", GrColo_SP$group), fillOpacity=1, popup = GrColo_SP$Popup, group=names(mod_out)[SP])
  }

  ### Add control layer per species
  Leaf <- Leaf %>%
    hideGroup(., group=names(mod_out)) %>%
    addPolygons(data=st_buffer(st_transform(windfarms, st_crs(4326)), 1000), fill="black", col="black", fillOpacity=1, label=paste0("Parc éolien : ", windfarms$NAME)) %>%
